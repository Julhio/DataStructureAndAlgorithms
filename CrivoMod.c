 ...
 * @brief Sieve of Eratosthenes OpenMP
 * @version 0.1
 * @date 2020-09-08
 * 
...
/****************************************************************************/
#define CACHE_SIZE_L1 (32*1024)
#define CACHE_SIZE_L2 (256*1024)
...
void CrivoBlock(int *primos, long indMax, long sqrtMax){
  long blocoTamanho = 0;
  int *blocoPrimos;
  
  //blockSize deve ser menor ou igual a CACHE_SIZE_L1
  if(indMax < CACHE_SIZE_L1) blocoTamanho = indMax;
  else blocoTamanho = CACHE_SIZE_L1;

  //Inicializa blocoPrimos de tamanho CACHE_SIZE_L1
  blocoPrimos = (int *) malloc(blocoTamanho*sizeof(int));

  //divide quantidade de números em blocos de tamanho até CACHE_SIZE_L1
  for (long blocoBaixo = 0; blocoBaixo <= indMax; ){
    // Calcula o endereço da parte alta para o vetor blocoPrimos
    long blocoAlto = blocoBaixo + blocoTamanho - 1; 
    if(blocoAlto >= indMax) blocoAlto = indMax;

    #ifdef _DEBUG
      prPrimos(primos, blocoTamanho);
    #endif

    // Copia os valores presentes no vetor primos deslocado para o blocoPrimos
    //*blocoPrimos = *(primos + blocoBaixo); //Copia utilizando endereço e trunca
    //for(long i=0; i <= (blocoAlto - blocoBaixo); i++) // Até o valor que falta  blocoAlto - blocoBaixo
    //  blocoPrimos[i] = primos[i + blocoBaixo + 1]; // 
    memcpy(blocoPrimos, (primos + blocoBaixo), sizeof(int)*(blocoAlto - blocoBaixo));
    ...
    #ifdef _DEBUG
      printf("\nblocoPrimos antes do Crivo:\n");
      prPrimos(blocoPrimos, blocoTamanho);
    #endif
    // Calcula quantidade de primos utilizando blocoPrimos
    Crivo(blocoPrimos, blocoAlto, sqrtMax);
    
    #ifdef _DEBUG
      printf("\nblocoPrimos depois do Crivo:\n");
      prPrimos(blocoPrimos, blocoTamanho);
    #endif

    // Desloca primos e copia os valores presentes no vetor blocoPrimos ele 
    //*(primos + blocoBaixo) = *blocoPrimos;
    //for(long i=0; i <= (blocoAlto - blocoBaixo + 1); i++) // Até o valor que falta  blocoAlto - blocoBaixo
    //  primos[i + blocoBaixo] = blocoPrimos[i];
    memcpy((primos + blocoBaixo), blocoPrimos, sizeof(int)*(blocoAlto - blocoBaixo));
    ...
    if(blocoAlto == indMax) break; //Se blocoAlto igual a indMax-1, termina.
    // Incrementa em blocoTamanho o endereço baixo do vetor primos para deslocar
    blocoBaixo += blocoTamanho; 
  }
}
/****************************************************************************/
...
int main(int argc, char *argv[]) {
  ...
  //initPrimosCrescente(primos, indMax);
#ifdef _DEBUG
  ...
  CrivoBlock(primos, indMax, sqrtMax);
